...
const AGENT_ALLOWLIST = [
  ...
  // content: offentlig innhold (GET), upsert (PUT /:nokkel)
  { method: 'GET', re: /^\/api\/content\/?$/ },
  { method: 'PUT', re: /^\/api\/content\/[^/]+\/?$/ },
  // health: åpen uansett, men adapteren treffer den med token.
  { method: 'GET', re: /^\/api\/health\/?$/ },
  // FJERNET 2026-08-07 ved forenklingen ("kun forsiden"): oppføringene for
  // /api/meldinger, /api/admin/content og /api/regnskap/{timer,poster} — de
  // rutene finnes ikke lenger. En allowlist som peker på slettede ruter er
  // ufarlig i seg selv (ingenting å treffe), men den lyver om hva agenten kan
  // nå. Legg dem tilbake sammen med ruten hvis den kommer igjen.
];
