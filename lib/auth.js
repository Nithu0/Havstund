...
// Hopper over hvis req.user allerede er satt (f.eks. av agentAuth/service-token
// FØR denne i kjeden) — da skal ikke et tilfeldig Bearer-token tolkes som JWT.
...
// Agent-rollen (service-token, brain-integrasjon) slipper gjennom KUN på de
// rutene brain-adapteren faktisk bruker (lib/agent-auth allowlist). Alle andre
// ruter er uberørt — agent får 403 der, akkurat som en kunde. Admin/ansatt-
// oppførsel er uendret. lazy-require for å unngå sirkulær avhengighet.
function requireRole(...roller) {
  return (req, res, next) => {
    if (!req.user) return res.status(401).json({ error: 'Ikke innlogget' });
    if (req.user.rolle === 'agent') {
      const { agentTillattHer } = require('./agent-auth');
      if (agentTillattHer(req)) return next();
