...
// Bygger emne + ren tekst + branded HTML for "innloggingslenke"-e-posten (magisk
// lenke). Speiler byggMottattMelding-monsteret (samme retur-form { emne, tekst,
// html }). Lenken vises bade som klikkbar knapp OG som ren URL (noen klienter
// blokkerer knapper). URL-en escapes inn i href for aa hindre attributt-injeksjon.
function byggInnloggingslenkeMelding(opts) {
  const o = opts || {};
  const navn = o.navn;
  const lenke = o.lenke || '';
  const hilsen = navn ? ('Hei ' + navn + ',') : 'Hei,';

  const emne = 'Havstund — logg inn og bekreft bookingen din';

  const linjer = [
    hilsen,
    '',
    'Takk for bookingen din! Vi har opprettet en konto til deg saa du kan foelge',
    'og bekrefte bookingene dine paa Min side.',
    '',
    'Klikk paa lenken under for aa logge inn — den er personlig og gyldig i 7 dager:',
  ];

  // Ren tekst: hilsen + intro + selve lenken paa egen linje.
  const tekst = linjer.concat(['', lenke, '', 'Vennlig hilsen', 'Havstund']).join('\n');

  const lenkeEsc = escapeHtml(lenke);
  const innhold =
    '<p style="margin:0 0 12px;">'
    + linjer.map((l) => (l === '' ? '<br>' : escapeHtml(l))).join('<br>')
    + '</p>'
    + '<p style="margin:20px 0;text-align:center;">'
    + '<a href="' + lenkeEsc + '" style="display:inline-block;background:#2a6f9e;color:#ffffff;'
    + 'text-decoration:none;padding:12px 28px;border-radius:8px;font-size:15px;font-weight:bold;">'
    + 'Logg inn</a></p>'
    + '<p style="margin:0 0 12px;font-size:13px;color:#5b6b78;">'
    + 'Virker ikke knappen? Kopier denne adressen inn i nettleseren:<br>'
    + '<a href="' + lenkeEsc + '" style="color:#2a6f9e;">' + lenkeEsc + '</a></p>'
    + '<p style="margin:16px 0 0;">Vennlig hilsen<br>Havstund</p>';
  const html = layout(innhold);

  return { emne, tekst, html };
}

/* Sender en magisk innloggingslenke til kunden (opprettet konto ved booking).
   FIRE-AND-FORGET — kaster ALDRI. Samme retur-kontrakt som sendStatusEpost:
   { ok:true, messageId } | { ok:false, simulert:true } | { ok:false, error }. */
async function sendInnloggingslenke(til, opts) {
  if (!isConfigured()) return { ok: false, simulert: true, grunn: 'SMTP ikke konfigurert' };
  if (!til) return { ok: false, error: 'mangler mottakeradresse' };

  try {
    const transport = lagTransport();
    const { emne, tekst, html } = byggInnloggingslenkeMelding(opts);
    const info = await transport.sendMail({
      from: fraAdresse(),
      replyTo: svarTilAdresse(),
      to: til,
    ...
    console.error('E-post-utsending (innloggingslenke) feilet:', e && e.message ? e.message : String(e));
    return { ok: false, error: e && e.message ? e.message : String(e) };
  }
}

module.exports = { isConfigured, statusTekst, sendStatusEpost, byggMelding, byggMottattMelding, layout, byggIcs, sendBookingMottatt, byggNyMeldingMelding, sendNyMelding, byggInnloggingslenkeMelding, sendInnloggingslenke };
